...
# write.table(training_output, file=paste(file_location,"training_output.csv",sep=""), quote=F, row.names=F, col.names=F, ,sep=",")
...
# logistic regression: use multinom or glmnet; glm works on for 2 classes 
set.seed(1)
lasso_model <- train(y=training_output, x=final_training_set, trControl=train_control, method = "glmnet",family = "multinomial",tuneGrid=expand.grid(alpha=1,lambda=0.001),preProcess = c("center", "scale","pca"))
